...
class ModerationCog(commands.Cog, name="Moderation Commands"):
    ...
    async def kick(self, ctx, member: discord.Member, *, reason=None):
        ...
        if ctx.author.id == member.id:
            kick_fun_embed = discord.Embed(
                ...
                description=f"{ctx.author.mention}, Looks like you need to visit a psychiatrist\n\
                    Here is the [contact info](https://www.youtube.com/watch\
        ...
        try:
            ...
            kick_embed.add_field(name="Kicked Member 👢",
    ...
    async def ban(self, ctx, member: discord.Member, *, reason=None):
        ...
        try:
            ...
            ban_embed.add_field(name="Banned Member 🚫",
                                value=member.display_name)
            ban_embed.add_field(name="Banned By", value=ctx.author.mention)
            ban_embed.set_footer(text=f"Reason: {reason}")
            ban_embed.set_thumbnail(url="https://media.tenor.co/videos\
                /16d1dd77408db03a6c78210391957fc5/mp4")

            await ctx.send(embed=ban_embed)
        except Exception as e:
            print(e)

    @commands.command(name="clear", usage="<number of messages> | default 10")
    ...
    async def clear(self, ctx, amount=10):
        ...
        clear_embed = discord.Embed(
            ...
            description=f"I've purged {amount} messages | \
                **{ctx.author.name}**",
            color=0x00FF00  # Green
        )
